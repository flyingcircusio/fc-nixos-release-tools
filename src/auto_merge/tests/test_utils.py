...
def config():
    yield Config(
        ...
        monitoring_review=MonitoringReviewConfig(
            names=["platform-dev"], notification_cutoff_hour=15
        ),
    )
...
def test_next_production_merge(config, now):
    ...
    assert next_production_merge(config) == datetime(
        2024, 11, 28, 0, 0, 0, tzinfo=ZoneInfo("Europe/Berlin")
    )

    # On the same day as the production merge
    now.return_value = datetime(
        2024, 11, 28, 14, 0, 0, tzinfo=ZoneInfo("Europe/Berlin")
    )
    assert next_production_merge(config) == datetime(
        2024, 12, 5, 0, 0, 0, tzinfo=ZoneInfo("Europe/Berlin")
    )
